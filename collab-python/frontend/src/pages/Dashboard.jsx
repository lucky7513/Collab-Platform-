  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-primary)' }}>
      ...
      <aside style={{ width: 240, background: 'var(--bg-secondary)', borderRight: '1px solid var(--border)', padding: '24px 16px', display: 'flex', flexDirection: 'column', gap: 8, flexShrink: 0 }}>
        ...
        <button style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 'var(--radius-sm)', padding: '10px 14px', cursor: 'pointer', fontSize: 14, fontWeight: 600, fontFamily: 'Outfit, sans-serif', marginBottom: 4 }} onClick={createDocument} disabled={creating}>
          <span style={{ fontSize: 18, lineHeight: 1 }}>+</span>
          {creating ? 'Creating...' : 'New Document'}
        </button>

        <button style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg-hover)', color: 'var(--text-primary)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '10px 14px', cursor: 'pointer', fontSize: 14, fontWeight: 500, fontFamily: 'Outfit, sans-serif', marginBottom: 8 }} onClick={joinWithCode}>
          🔗 Join with Code
        </button>

        <div style={{ marginTop: 8 }}>
          <p style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-muted)', letterSpacing: '0.1em', padding: '0 8px', marginBottom: 4 }}>WORKSPACE</p>
          <div style={{ padding: '8px 10px', borderRadius: 'var(--radius-sm)', fontSize: 13, color: 'var(--text-primary)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, background: 'var(--accent-dim)' }}>📄 All Documents</div>
        </div>
