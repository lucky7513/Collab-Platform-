    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-primary)' }}>
      <aside style={{ width: 240, background: 'var(--bg-secondary)', borderRight: '1px solid var(--border)', padding: '24px 16px', display: 'flex', flexDirection: 'column', gap: 8, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 8px', marginBottom: 16 }}>
          <div style={{ width: 30, height: 30, background: 'var(--accent-dim)', border: '1px solid var(--accent)', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>
            ⚡
          </div>
          <span style={{ fontSize: 18, fontWeight: 700 }}>Collab</span>
        </div>
        <button style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 'var(--radius-sm)', padding: '10px 14px', cursor: 'pointer', fontSize: 14, fontWeight: 600, fontFamily: 'Outfit, sans-serif' }} onClick={createDocument} disabled={creating}>
          <span style={{ fontSize: 18, lineHeight: 1 }}>+</span>
          {creating ? 'Creating...' : 'New Document'}
        </button>
        <button style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg-hover)', color: 'var(--text-primary)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '10px 14px', cursor: 'pointer', fontSize: 14, fontWeight: 500, fontFamily: 'Outfit, sans-serif' }} onClick={joinWithCode}>
          Join with Code
        </button>
        <div style={{ marginTop: 8 }}>
          <p style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-muted)', letterSpacing: '0.1em', padding: '0 8px', marginBottom: 4 }}>WORKSPACE</p>
          <div style={{ padding: '8px 10px', borderRadius: 'var(--radius-sm)', fontSize: 13, color: 'var(--text-primary)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, background: 'var(--accent-dim)' }}>
            All Documents
          </div>
        </div>
      ...
      <main style={{ flex: 1, padding: '40px 48px', overflow: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 32 }}>
          <div>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>{documents.length} documents</p>
          </div>
          <button style={{ background: 'var(--bg-card)', color: 'var(--text-primary)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '10px 18px', cursor: 'pointer', fontSize: 14, fontFamily: 'Outfit, sans-serif' }} onClick={createDocument}>
            + New Document
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', paddingTop: 80 }}>
            ...
            <h2 style={{ fontSize: 20, fontWeight: 600, marginBottom: 8 }}>No documents yet</h2>
            <button style={{ background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 'var(--radius-sm)', padding: '10px 24px', cursor: 'pointer', fontSize: 14, fontWeight: 600, fontFamily: 'Outfit, sans-serif' }} onClick={createDocument}>Create Document</button>
          </div>
        ) : (
